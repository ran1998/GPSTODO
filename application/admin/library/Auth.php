...
class Auth extends \fast\Auth
{
    ...
    public function getAdminChilrensId($relativeId = null)
    {
        $relativeId = $relativeId == null ? $this->id : $relativeId;
        if ($this->isSuperAdmin()) {
            $condition = array();
        } else {                
            $condition = array('pid' => $relativeId);
        }
        $adminChilrensId = collection(model('admin')->field('id')->where($condition)->select())->toArray();
        $chilrenIds = array_column($adminChilrensId, 'id');
        array_push($chilrenIds, $relativeId);

        $this->adminChilrensId = $chilrenIds;
        if (empty($this->adminChilrensId)) {
        }
        return $this->adminChilrensId;
    }
    /**
     * 管理员登录
     *
